...
public class ExpenseFileParser {
	...
	public static List<Map<Integer, Double>> parseExpenses(List<String> expenseList) {
		...
		List<Map<Integer, Double>> parsedExpenses = new ArrayList<>();
		
		
		for (String expense : expenseList) {
			String[] parts = expense.split("[,:\\s]+");
			if(parts.length == 2) {
				try {
					// get month number and expense amount
                    int month = Integer.parseInt(parts[0]);
                    double amount = Double.parseDouble(parts[1]);
                    
                    // create a hashmap with month number as key and expense amount as value
                    ...
                    // Handle parsing exceptions here if required
                    e.printStackTrace();
                }
			}
		}
